...
namespace ABB.DAL.Production
{
    public class ProductMaterial
    {
        ...
        public bool DeleteCurrentDataByREQ(double ReqLOID, OracleTransaction zTrans)
        {
            return doDelete(" REQUISITION = " + ReqLOID.ToString() + " ", zTrans);
        }

        public bool DeleteCurrentData(string wherestr, OracleTransaction zTrans)
        {
            return doDelete(wherestr, zTrans);
        }
        ...
        private string sql_insert
        {
            get
            {
                string sqlz = "INSERT INTO " + tableName + " (LOID,CREATEBY,CREATEON,UPDATEBY,UPDATEON,PRODUCT,REQUISITION,MASTER,USEQTY,WASTEQTYMAT,WASTEQTYMAN,RETURNQTY, CHANGEQTY,UNIT,BOM,YIELDMAM,YIELDMAT,REMARK,ACTIVE)VALUES(";
        ...
        private bool doGetdata(string whText, OracleTransaction zTrans)
        {
            ...
            if (whText.Trim() != "")
            {
                ...
                try
                {
                    ...
                    if (zRdr.Read())
                    {
                        _OnDB = true;if (!Convert.IsDBNull(zRdr["LOID"])) _LOID = Convert.ToDouble(zRdr["LOID"]);
        ...
        {
            return doDelete(" LOID = " + PDRequest.ToString() + " ", zTrans);
        }

        public bool DeleteDataByReturn(double ReturnRequest, OracleTransaction zTrans)
        {
            return doDelete(" REQUISITION = " + ReturnRequest.ToString() + " ", zTrans);
        }

    ...
    public class PDProductDAL
    {
        ...
        private string sql_insert
        {
            get
            {
                ...
                sqlz += " '" + OracleDB.QRText(_PRODUCTTYPE) + "',";// PRODUCTTYPE";
                sqlz += "  " + _TOWAREHOUSE.ToString() + ",";// TOWAREHOUSE";
                sqlz += "  " + _LOSTQTY.ToString() + ",";// LOST";
                sqlz += " " + _QUARANTINEUNIT.ToString() + "";
                sqlz += " ) ";
                return sqlz;
            }
        }
        private string sql_update
        {
            get
            {
                ...
                //sqlz += " PRODUCETYPE  = '" + OracleDB.QRText(_PRODUCTTYPE) + "', ";
                //sqlz += " TOWAREHOUSE  = " + _TOWAREHOUSE.ToString() + ",";
                sqlz += " LOSTQTY  = " + _LOSTQTY.ToString() + ", ";
        ...
        private bool doInsert(OracleTransaction zTrans)
        {
            ...
            if (!_OnDB)
            {
                try
                {
                    ...
                    if (_LOTNO == "") { _LOTNO = ProductBarcodeDAL.GetLotNo(_CREATEBY, _PRODUCT, zTrans); }
            ...
            if (whText.Trim() != "")
            {
                ...
                try
                {
                    ...
                    if (zRdr.Read())
                    {
                        ...
                        if (!Convert.IsDBNull(zRdr["LOSTQTY"])) _LOSTQTY = Convert.ToDouble(zRdr["LOSTQTY"]);
        ...
        public bool DeleteDataByPDOrder(double PDOrder, OracleTransaction trans)
        {
            return doDelete("PDORDER = " + PDOrder.ToString(), trans);
        }

        public bool UpdateStatusByPDOrder(double PDOrder, string status, string userID, OracleTransaction zTrans)
        {
            bool ret = true;
            string sql = "UPDATE " + TableName + " SET PRODSTATUS = '" + status + "', ";
            sql += "UPDATEBY = '" + userID + "', UPDATEON = SYSDATE ";
            sql += "WHERE PORDER = " + PDORDER.ToString() + " ";
            try
            {

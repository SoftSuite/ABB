...
namespace ABB.DAL
{
    public class ProcessDAL
    {
        ...
        private string sql_insert
        {
            get
            {
                string sqlz = "INSERT INTO " + tableName + " (LOID,CODE,PROCESS,PRODUCT,ACTIVE,CREATEBY,CREATEON,UPDATEBY,UPDATEON,RADIATION)VALUES(";
                sqlz += "  " + _LOID.ToString() + ",";// LOID";
                sqlz += " '" + OracleDB.QRText(_CODE) + "',";// CODE";
                sqlz += " '" + OracleDB.QRText(_PROCESS) + "',";// PROCESS";
        ...
        private string sql_update
        {
            get
            {
                ...
                sqlz += " PROCESS  = '" + OracleDB.QRText(_PROCESS) + "', ";
            ...
            if (whText.Trim() != "")
            {
                ...
                try
                {
                    ...
                    if (zRdr.Read())
                    {
                        ...
                        if (!Convert.IsDBNull(zRdr["PROCESS"])) _PROCESS = zRdr["PROCESS"].ToString();
        ...
        public bool DeleteDataByProduct(double product, OracleTransaction trans)
        {
            return doDelete("PRODUCT = " + product.ToString(), trans);
        }

        public bool GetDataByProduct(double product, OracleTransaction zTrans)
        {
            return doGetdata(" PRODUCT = " + product.ToString() + " ", zTrans);
        }
    }
}

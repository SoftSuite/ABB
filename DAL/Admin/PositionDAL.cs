...
namespace ABB.DAL.Admin
{
    public class PositionDAL
    {
        ...
        private string sql_insert
        {
            get
            {
                string sqlz = "INSERT INTO " + tableName + " (LOID,CREATEBY,CREATEON,UPDATEBY,UPDATEON,NAME,CODE)VALUES(";
        ...
        {
            string sql = "SELECT * FROM "+ tableName +" WHERE NAME = '" + name + "' AND LOID <> " + loid + " ";
            DataTable dt = OracleDB.ExecListCmd(sql);
            if (dt.Rows.Count > 0)
            {
                return false;

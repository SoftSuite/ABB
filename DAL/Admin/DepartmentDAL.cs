...
namespace ABB.DAL.Admin
{
    public class DepartmentDAL
    {
        ...
        private string sql_insert
        {
            get
            {
                string sqlz = "INSERT INTO " + tableName + " (LOID,CREATEBY,CREATEON,UPDATEBY,UPDATEON,TNAME,EFDATE,EPDATE,CODE)VALUES(";
        ...
        {
            string sql = "SELECT * FROM DEPARTMENT WHERE CODE = '" + code + "' AND LOID <> " + loid + " ";
        ...
        {
            string sql = "SELECT * FROM DEPARTMENT WHERE TNAME = '" + name + "' AND LOID <> " + loid + " ";
            DataTable dt = OracleDB.ExecListCmd(sql);
            if (dt.Rows.Count > 0)
            {
                return false;

...
namespace ABB.DAL
{
    public class CtrlDeliveryDAL
    {
        ...
        private string tableName = "CTRLDELIVERY";
        ...
        private string sql_insert
        {
            get
            {
                string sqlz = "INSERT INTO " + tableName + " (LOID,CODE,DELIVERYDATE,CARNO,DRIVERNAME,CREATEBY,CREATEON,UPDATEBY,UPDATEON)VALUES(";
                sqlz += "  " + _LOID.ToString() + ",";// LOID";
                sqlz += " '" + OracleDB.QRText(_CODE) + "',";// CODE";
                sqlz += " " + OracleDB.QRDateTime(_DELIVERYDATE) + ",";// DELIVERYDATE";
                sqlz += " '" + OracleDB.QRText(_CARNO) + "',";// CARNO";
                sqlz += " '" + OracleDB.QRText(_DELIVERYNAME) + "',";// DELIVERYNAME";
        ...
        private string sql_update
        {
            get
            {
                ...
                sqlz += " CARNO  = '" + OracleDB.QRText(_CARNO) + "', ";
                sqlz += " DRIVERNAME  = '" + OracleDB.QRText(_DELIVERYNAME) + "', ";
        ...
        private bool doInsert(OracleTransaction zTrans)
        {
            ...
            if (!_OnDB)
            {
                try
                {
                    ...
                    _CODE = OracleDB.GetRunningCode(TableName, TableName, zTrans);
            ...
            if (whText.Trim() != "")
            {
                ...
                try
                {
                    ...
                    if (zRdr.Read())
                    {
                        ...
                        if (!Convert.IsDBNull(zRdr["DELIVERYDATE"])) _CREATEON = OracleDB.DBDate(zRdr["DELIVERYDATE"]);
                        if (!Convert.IsDBNull(zRdr["CARNO"])) _CARNO = zRdr["CARNO"].ToString();
                        if (!Convert.IsDBNull(zRdr["DRIVERNAME"])) _DELIVERYNAME = zRdr["DRIVERNAME"].ToString();

...
{
    public class TodoListProductAnalysisFlow
    {
        string _error = "";
        double _STLOID = 0;
        ...
        public DataTable GetPDRequestList(QCAnalysisSearchData data)
        {
            ...
            sql += "CASE STATUS WHEN '" + Constz.Requisition.Status.SendQC.Code + "' THEN '" + Constz.Requisition.Status.SendQC.Name + "' ";
            sql += "WHEN '" + Constz.Requisition.Status.ReturnQC.Code + "' THEN '" + Constz.Requisition.Status.ReturnQC.Name + "' ";
            sql += "ELSE '' END AS STATUSNAME, ";
            sql += "CASE STATUS WHEN '" + Constz.Requisition.Status.SendQC.Code + "' THEN '" + Constz.Requisition.Status.SendQC.Rank + "' ";
            sql += "WHEN '" + Constz.Requisition.Status.ReturnQC.Code + "' THEN '" + Constz.Requisition.Status.ReturnQC.Rank + "' ";
            sql += "ELSE '' END AS RANK ";
            sql += "FROM V_TODOLIST_QC ) A ";
            sql += (whereString == "" ? "" : "WHERE STATUSVAL LIKE '%QC%' AND " + whereString);
